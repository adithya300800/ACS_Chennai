...
| `__tests__/inspection-fresh-admin.test.js` | All 5 tests in 1 describe (new file) | S3-9: closes the stale-JWT-admin-claim gap on `POST /api/inspection`. Pinned: status=OPEN skips the fresh check (zero employee.findUnique calls); status=CLOSED + DB confirms isAdmin=true → 201; status=CLOSED + DB says isAdmin=false (freshly demoted) → 403 STATUS_ADMIN_ONLY (the load-bearing test); employee row gone → 403; DB throws → 500 surfaced, inspectionRecord.create never called. | REPORT-S3-9 |
| `__tests__/admin-attendance-digest-idempotency.test.js` | All 8 tests across 6 describes (new file) | S3-11: closes the manual-re-fire double-email gap on `POST /api/internal/attendance/digest/run`. Pinned: first fire → 3 EmailLog SENT + 3 AdminDigestRun SENT with emailLogId linked; second fire on the same date → idempotentSkips=3, no new EmailLog rows, no sendEmail calls; different `?date=` → 3 sends (per-date key); emailEnabled=false admin → AdminDigestRun status=SKIPPED_OPT_OUT (still audited); typeMutes.ADMIN_ATTENDANCE_DAILY → status=SKIPPED_TYPE_MUTED; P2002 from claim returns 200 (NOT 500); FAILED path stamps AdminDigestRun.status=FAILED with errorMessage populated. Also added the adminDigestRun mock surface to the legacy admin-attendance-digest.test.js makePrisma factory so the pre-existing suite still passes against the new contract. | REPORT-S3-11 |
...
| `__tests__/dpr.month-shortcut.test.js` | `no month param → falls through to the existing range/empty filter path`, `to=YYYY-MM-DD alone still works` | LPR-029 (round-29) | Both tests pass in isolation (`npm test -- --testPathPattern=dpr.month-shortcut` → 7/7 green) but fail when run as part of the full suite. Pollution source: `requireAuth` middleware loads a `revokedToken` prisma model reference in the read-through revocation cache; `warmup.test.js` and `auth.test.js` import `./src/middleware/auth` and run before this file, so by the time these tests mount the route the auth middleware throws "Cannot read properties of undefined (reading 'findUnique')" before the route handler runs. The route code is correct (5 sibling tests in the same file pass, plus all dpr.update/dpr.stats/dpr.fanout-scope tests). Re-enable in round-29.5 by splitting the mock into a `jest.doMock` of `requireAuth` for this file only. |
